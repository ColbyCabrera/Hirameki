...
class InitialActivityTest : RobolectricTest() {
    ...
    @Test
    fun legacy_accessible_folder_uses_public_legacy_permissions() {
        val expectedPermissions =
            arrayOf(
                android.Manifest.permission.READ_EXTERNAL_STORAGE,
                android.Manifest.permission.WRITE_EXTERNAL_STORAGE,
            )
        val withoutManageExternalStorage =
            selectAnkiDroidFolder(
                canManageExternalStorage = false,
                currentFolderIsAccessibleAndLegacy = true,
            )
        val withManageExternalStorage =
            selectAnkiDroidFolder(
                canManageExternalStorage = true,
                currentFolderIsAccessibleAndLegacy = true,
            )

        assertThat(withoutManageExternalStorage, instanceOf(PublicFolder::class.java))
        assertThat(withManageExternalStorage, instanceOf(PublicFolder::class.java))

        assertThat(
            (withoutManageExternalStorage as PublicFolder).requiredPermissions.asIterable(),
            contains(*expectedPermissions),
        )
        assertThat(
            (withManageExternalStorage as PublicFolder).requiredPermissions.asIterable(),
            contains(*expectedPermissions),
        )
    }
    ...
    @Test
    fun non_legacy_folder_without_external_manager_uses_app_private_folder() {
        assertThat(
            selectAnkiDroidFolder(
                canManageExternalStorage = false,
                currentFolderIsAccessibleAndLegacy = false,
